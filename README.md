...
| `y` / `n` / `a` | Answer a permission prompt: once / refuse / always. **While a prompt is up the prompt owns the keyboard, and these are matched against every character you type** (case-insensitively) — nothing reaches the input box, so typing a slash command answers the prompt instead of running. Measured: `/keys` answers `y` on its fourth keystroke, `/init` answers `n` on its third, and `/agents` answers **`a` on its second** — and `a` is *always*, a grant recorded for that tool for the rest of the session, so every later call to it runs without asking. Read the prompt before you type |
