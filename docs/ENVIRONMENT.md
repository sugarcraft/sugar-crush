...
| `SUGARCRUSH_DISABLE_PROMPT_CACHE` | unset — breakpoints are enabled | Set to any value other than empty or `0` and `CacheBreakpoints::disabledFromEnvironment()` reports the switch on; a disabled instance runs `apply()`'s full input audit (shape throws, the ephemeral wipe, the over-cap breach on foreign marks) and then adds **zero** new `cache_control` breakpoints — preserved marks from other mechanisms ride on untouched. **Dormant by design at this tree**: nothing in `src/` or `bin/` consults it yet. `CacheBreakpoints` itself ships unwired pending the licensing decision on prompt caching (see its class docblock, WHAT THIS TYPE IS FOR TODAY / THE KILL SWITCH), so setting this variable today changes no behaviour you can observe; the reader's semantics are pinned by tests so the wiring step lands on a switch that already works. |
